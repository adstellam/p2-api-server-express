 ...
 */
 ...
 router.get('/', function(req, res, next) {
     if (!req.query) {
         pgPool.query('SELECT * FROM fields;', (err, result) => { 
             if (err) 
                 next(new Error(err));
             else {
     ...
     if (req.query.oid) {
         pgPool.query('SELECT * FROM fields WHERE oid = $1;', [req.query.oid], (err, result) => { 
             if (err) 
                 next(new Error(err));
             else {
                 console.log(result.rows);
 ...
 router.get('/:fid', function(req, res, next) {
     pgPool.query('SELECT * FROM fields WHERE id = $1;', [req.params.fid], (err, result) => {
         if (err) 
             next(new Error(err));
         else {
 ...
 router.patch('/:fid', function(req, res, next) {
